...
<div class="flex justify-center items-center min-h-full w-full mt-24">
    <form action="{{ route('changepassword') }}" method="post" class="flex flex-col w-1/4 space-y-4">
        ...
        @csrf
        <input class="border p-4 @error('old_pass')
            is-invalid
        @enderror" type="password" name="old_pass" required autofocus placeholder="Enter old password">
        ...
        <input class="border p-4 @error('new_pass')
            is-invalid
        @enderror" type="password" name="new_pass" required placeholder="Enter new password">
        @error('new_pass')
        <div class="invalid-feedback text-red-500">
            {{$message}}
        </div>
        ...
        <input class="border p-4 @error('password')
            is-invalid
        @enderror" type="password" name="new_pass_re" required placeholder="Confirm new password">
        @error('new_pass_re')
        <div class="invalid-feedback text-red-500">
            {{$message}}
        </div>
